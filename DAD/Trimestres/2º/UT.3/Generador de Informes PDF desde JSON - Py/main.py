...
def limpiar_tabla():
    ...
    for item in tree.get_children():
        tree.delete(item)   # Elimina cada elemento (fila) del "Treeview".
...
def cargar_productos(termino = ""):
    ...
    if productos is not None:
        productos_actuales = productos  # Actualiza la variable global con los productos obtenidos. Es decir, guarda los productos reunidos/obtenidos en la lista global
        return True
...
def buscar_productos_por_termino():
    ...
    # Muestra un mensaje temporal de carga en la tabla mientras se obtienen los datos.
    tree.insert("", "end", values =("Cargando...", "", "", "", "", "", "", "", "")) # Inserta una fila temporal indicando que se están cargando los datos.
    root_windows.update()                                                           # Actualiza la ventana para mostrar el mensaje de carga.
...
def exportar_pdf():
    ...
    try:
        ...
        messagebox.showinfo("Exportación completada", f"El PDF se ha generado correctamente.\n\n\tNombre del Archivo: {nombre_archivo}\n\tUbicación del Archivo: {carpeta}\n\tFormato del Archivo: PDF\n\n¡Gracias por utilizar la aplicación 'Buscador de Productos - DummyJSON'!")

    except Exception as ex:
        messagebox.showerror("Error de exportación", f"Error inesperado al intentar generar, cargar y/o guardar el PDF:\n{ex}")
...
btn_ver_todos.config(bg = "#27AE60", fg = "white", font = ("Arial", 10, "bold"), relief = "raised", bd = 3)        # Configura el estilo del botón (color de fondo, color de texto, fuente, relieve y borde).
btn_ver_todos.grid(row = 0, column = 3, padx = 5)                                                                    # Empaqueta/Muestra el botón en la cuadrícula (grid).

btn_exportar = tk.Button(frame_superior, text = "📄\nExportar a PDF", command = exportar_pdf)                 # Crea un botón para exportar los datos a PDF.
btn_exportar.config(bg = "#E74C3C", fg = "white", font = ("Arial", 10, "bold"), relief = "raised", bd = 3)  # Empaqueta/Muestra el botón en la cuadrícula (grid).
btn_exportar.grid(row = 0, column = 4, padx = 5)                                                              # Empaqueta/Muestra el botón en la cuadrícula (grid).
...
tree.heading("Valoración", text = "Valoración")
